...
>>>>>>> 8a7be53146125bb202b80bb7f392c4c06c72c047
}
...
<<<<<<< HEAD
      ...
      if(lid == 0){
         ...
         length = input_pos[curr_pos + 1] - pos[curr_pos];
         first_char = (input_string[pos[curr_pos]] == OPEN);
         result_sizes[curr_pos / 2] = 0;
      ...
      while(elems_scanned < length){
         ...
         separators[lid] = ((lstring[lid] == SEP) &&
                           ((first_char) ? (function & 2) >> 1) : (function & 1)));
         barrier(CLK_LOCAL_MEM_FENCE);
...
=======
		...
		if(lid == 0){
			...
			len = pos[curr_pos + 1] - pos[curr_pos];
			first_char = (input_string[pos[curr_pos]] == OPEN);
